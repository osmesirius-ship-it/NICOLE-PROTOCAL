...
### Modular UI console
- A lightweight, modular UI is provided via Vite to visualize governance layers, modules, and end-to-end workflows.
- It uses native ES modules (no backend required) and can be extended with live data sources.
...
- **Static analysis and formatting:**
  ...
  make fmt
  ```
- **UI module tests (Vitest):**
  ```bash
  npm test
  ```
- **End-to-end (sample) workflow test:** exercises mandate submission through execution adapters and evidence collection.
  ```bash
  make e2e
  ```
