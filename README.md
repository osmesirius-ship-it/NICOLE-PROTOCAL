...
## Setup Requirements
- **Shell:** Commands assume `zsh` on macOS/Linux. Ensure your `PATH` includes the repo's `node_modules/.bin` after installs (e.g., `export PATH="$PWD/node_modules/.bin:$PATH"`).
- **Runtime:** Node.js 18+ (UI + scripts) and Python 3.11+ (for future SDK references). Align your version managers (`asdf`, `fnm`, `pyenv`) in `~/.zshrc` so subshells inherit the right versions.
- **Tooling:** Git and Make for workflow shortcuts; Docker is optional for isolated runs.
...
- **Environment Variables:**
  ...
  - `NICOLE_STATIC_ROOT` (optional; override the path served by the dev/preview server when pointing at a built bundle)

## Terminal quickstart (step-by-step)
Use these exact commands in a `zsh` shell to explore the project from scratch:

1. **Clone and enter the repo**
   ```bash
   git clone <repo-url>
   cd NICOLE-PROTOCAL
   pwd   # prints the path so you know where you are
   ls    # lists files like README.md, src/, scripts/
   ```

2. **Install dependencies** (writes `node_modules/` locally)
   ```bash
   npm install
   ```

3. **Run the modular UI** (dev server from source)
   ```bash
   npm run dev
   ```
...
4. **Build the static bundle** (writes files you can host anywhere)
   ```bash
   npm run build
   ls dist   # shows the generated index.html and src/ assets
   ```

5. **Preview the built bundle** (serves `dist/` instead of source)
   ```bash
   npm run preview
   ```

6. **Run the tests** (simple Node assertions, no browsers)
   ```bash
   npm test
   ```

7. **Edit files and re-run commands**
   - UI code: `src/main.js`, `src/components/`, `src/state/`, `src/style.css`.
   - HTML entry: `index.html` at the repo root.
   - Scripts: `scripts/dev-server.js`, `scripts/build.js`.
   - Tests: `test/derive.test.js`.
   After edits, re-run `npm run dev`, `npm run build`, or `npm test` as needed.
...
**File locations (by workflow):**
- Authoring: edit `index.html`, `src/main.js`, component files in `src/components/`, shared state in `src/state/`, and styling in `src/style.css`.
- Scripts: dev/build/preview helpers live in `scripts/` (`dev-server.js`, `build.js`).
- Tests: assertions for derived governance state live in `test/derive.test.js`.
- Build output: `npm run build` writes the static bundle to `dist/`; `npm run preview` can serve that folder directly.
